...
| `behavior` | ClawHub Observatory Behavior | [docs](observatory.md) | Paired baseline/exercise runtime evidence from a separately isolated disposable OpenClaw environment. | `CLAWSCAN_BEHAVIOR_CONFIG` | build `cmd/observatory`; run Clawscan with `--sandbox off` |
