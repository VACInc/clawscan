...
func TestRunCommandPrintsHelp(t *testing.T) {
	...
	for _, want := range []string{
		...
		"agentverus, aig, behavior, cisco, clawscan-static, skillspector, snyk, socket, virustotal",
		"Required environment variables:",
		"aig: LLM_API_KEY or OPENAI_API_KEY",
		"behavior: CLAWSCAN_BEHAVIOR_CONFIG",
...
func TestRunCommandScansPluginTargetAndSkipsSkillOnlyScanner(t *testing.T) {
	...
	writePlugin(t, target, "observatory-probe")
	out := filepath.Join(dir, "run.json")

	stdout := captureStdout(t, func() {
		if err := run([]string{target, "--scanner", "clawscan-static", "--output", out}, []string{}); err != nil {
			t.Fatal(err)
		}
	})
	...
	if artifact.Target.ID != "observatory-probe" {
		t.Fatalf("target id = %q", artifact.Target.ID)
	}
	static := artifact.Scanners["clawscan-static"]
	if static.Status != "skipped" || !strings.Contains(static.Error, "does not support plugin targets") {
...
func TestRunCommandDoesNotAutoDiscoverPlugins(t *testing.T) {
	...
	writePlugin(t, filepath.Join(dir, "skills", "probe-plugin"), "observatory-probe")
	t.Chdir(dir)
