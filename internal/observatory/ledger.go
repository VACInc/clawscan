...
type auditLedgerDocument struct {
	...
	RecorderObserved *bool        `json:"recorderObserved"`
...
// parseToolAuditLedger fails closed on a malformed direct-export receipt.
...
func parseToolAuditLedger(data []byte) (toolAuditCapture, error) {
	...
	if document.RecorderObserved == nil || !*document.RecorderObserved {
		return toolAuditCapture{}, errors.New("audit recorder lifecycle was not observed")
	}
...
func validateAuditEvent(event auditEvent) error {
	...
	if event.Action == "tool.action.finished" {
		expectedError := toolCallErrorCodeByState[event.Status]
...
func toolCallCoverage(total int, published int, missing int, truncated bool) (string, string) {
	reasons := []string{
		"OpenClaw audit persistence is best-effort, so recorded rows cannot prove complete tool-call coverage.",
	}
	if total == 0 {
		reasons = append(reasons, "No tool-call rows were observed.")
	}
	if missing > 0 {
